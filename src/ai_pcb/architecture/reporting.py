...
def render_phase3_report(state: DesignState) -> str:
    ...
    for candidate in state.component_candidates:
        ...
        if evaluation is not None:
            ...
            lines.extend(
                f"      unresolved: {item}" for item in evaluation.unresolved_trade_offs
            )
    ...
    if state.computational_budget is not None:
        ...
        for scenario in state.computational_budget.scenarios:
            lines.append(
                f"  - {scenario.label}; DESIGN_EXPLORATION={scenario.design_exploration}"
            )
            lines.extend(f"      assumption: {item}" for item in scenario.assumptions)
            lines.extend(
                f"      operations: {item}"
                for item in scenario.operations_per_second.conditions
            )
    if state.latency_budget is not None:
        ...
        for contribution in state.latency_budget.contributions:
            lines.append(
                f"  - {contribution.contributor.value}: "
                f"{contribution.latency.status.value}"
            )
            lines.extend(
                f"      {condition}" for condition in contribution.latency.conditions
            )
    lines.append("Risks:")
    if state.architecture is not None:
